...
def test_empty_content_retries_without_reasoning_or_extra_output_budget(monkeypatch) -> None:
    class FakeResponse:
        status_code = 200
        text = ""
    ...
    assert FakeAsyncClient.payloads[1]["thinking"] == {"type": "disabled"}
    assert "reasoning_effort" not in FakeAsyncClient.payloads[1]
    assert FakeAsyncClient.payloads[1]["max_tokens"] == 12_000
...
def test_agent_generation_profile_is_sent_for_selected_agent(monkeypatch) -> None:
    class FakeResponse:
        status_code = 200
        text = ""
    ...
    settings = Settings.from_mapping(
        {
            ...
            "model": "custom-model",
            "agent_generation": {
                "reviewer": {"temperature": 0.35, "top_p": 0.72, "top_k": 24}
            },
        }
    )

    asyncio.run(
        DeepSeekProvider(settings).generate_json(
            system_prompt="审查",
            user_prompt="待审正文",
            output_model=ReviewReport,
            agent_role="reviewer",
    ...
    assert FakeAsyncClient.payload["temperature"] == 0.35
    assert FakeAsyncClient.payload["top_p"] == 0.72
    assert FakeAsyncClient.payload["top_k"] == 24

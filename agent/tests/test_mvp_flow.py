...
def test_complete_mvp_flow(tmp_path: Path) -> None:
    ...
    assert [item["key"] for item in context["sections"]] == list("ABCDEFGHIJK")
...
def test_selection_revision_replaces_only_locked_range_and_keeps_history(tmp_path: Path) -> None:
    replacement = SelectionRevisionOutput(
        replacement="墨迹在灯下泛着尚未凝住的潮光",
        decision_summary=["只替换用户锁定的视觉描写", "前后因果与人物知识边界保持不变"],
    )
    provider = ScriptedProvider([make_plan(), make_draft(), replacement])
    engine = InkFlowEngine(provider, Settings(context_soft_tokens=20_000))
    root = tmp_path / "novel"
    ...
    selected = "墨水尚未干"
    start = before.index(selected)
    end = start + len(selected)

    result = asyncio.run(
        engine.revise_selection(
            root,
    ...
    assert after == before[:start] + replacement.replacement + before[end:]
    assert result["version"] == 2
    assert result["next_action"].startswith("重新审查")
    assert provider.calls[-1]["output_model"] == "SelectionRevisionOutput"
    assert "# 唯一允许替换的原文" in provider.calls[-1]["user_prompt"]
    assert StudioService(InkFlowProject(root)).db.list_versions("chapters/chapter_00001.draft.md")[0]["applied"] == 1
    annotations = StudioService(InkFlowProject(root)).read_document("chapters/chapter_00001.draft.md")["annotations"]
    assert annotations[0]["status"] == "resolved"


def test_selection_revision_rejects_stale_document_hash(tmp_path: Path) -> None:
    provider = ScriptedProvider([make_plan(), make_draft()])
    engine = InkFlowEngine(provider, Settings(context_soft_tokens=20_000))
    root = tmp_path / "novel"
    ...
    engine.create_project(root, make_brief())
    asyncio.run(engine.generate_plan(root))
    written = asyncio.run(engine.write_chapter(root, 1))
    before = Path(written["draft_path"]).read_text(encoding="utf-8")
    start = before.index("墨水尚未干")

    with pytest.raises(ValidationGateError, match="已经变化"):
        asyncio.run(
            engine.revise_selection(
                root,
                "chapters/chapter_00001.draft.md",

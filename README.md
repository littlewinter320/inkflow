...
Coordinator 是第四个正式 AI Agent，负责理解需求、建立任务单、选择已有工作流、安排下一步、汇总结果和提出必要的最小问题。用户明确要求时，它还可以通过 Novel Engine 修改白名单运行设置，并把旧值、新值和下一步建议返回到聊天中。它不写正文、不替 Reviewer 放行，也不替 Memory Keeper 提交正史。
...
0.6.5 在保留语音、模型参数、章节容差、自然描写和缓存优化的基础上，继续解决 Coordinator 不会给出下一步、长章节范围被截断、设置修改不透明和固定工作流缺少引导的问题：

- **Coordinator 先理解再给建议**：自然语言请求会生成可复核的路由理由、任务单和公开阶段；完成后弹出一个可关闭的“下一步建议”，只把推荐语句放回输入框，不会未经用户操作自动执行。状态查询和桌面工作流按钮也使用同一套引导。
- **用户要求时可改设置**：Coordinator 只接受白名单设置，支持模糊但明确的“把容错调到 30%”“Writer 温度调到 0.7”“思考强度调低”“关闭自动朗读”等说法。完成后在聊天中列出每一项“旧值 → 新值”，同值会明确说明没有变化；模型、正史和密钥不会被自然语言设置入口越权修改。
- **独立大纲生成器**：可以直接生成第 11～30 章或更长范围的大纲，章节号严格连续，结果单独保存到 `planning/outlines/`，不覆盖 `PLAN.md`、不生成草稿、不审查、不进入正史。
- **长范围不再被固定上限截断**：规划预览、批量草稿、篇章复审和独立大纲按用户给出的起止章处理，必要的输出预算随范围调整；正式写作仍按现有门禁逐章推进。
- **后台安装和项目打开互不阻塞**：MOSS/Qwen 依赖点击后进入后台队列，关闭设置不会取消；项目打开的轻量整理不会等待长任务，运行中新增消息会显示并在安全节点接收或排队。
- **公开过程更清楚**：聊天和协作台显示 Coordinator、Writer、Reviewer、Memory Keeper 的阶段、模型、文件和依据摘要；原始思维链不保存、不展示，避免把内部推理误当成小说内容。

- **语音组件安装全程可见**：MOSS 与 Qwen 安装会显示当前阶段、总体进度、已写入容量和简短状态。依赖安装、自动重试、模型下载、完成与失败使用同一张进度卡，不再把 pip 原始日志铺满通知窗口。
- **Windows 临时文件故障可恢复**：语音依赖改用墨流自己的临时目录和持久 pip 缓存。遇到 `pip-unpack` 中 wheel 文件被提前清理时，安装器会先自动重试一次；再次失败会保留已有组件、清理本次暂存目录，并在设置中给出可重试原因。已经下载进缓存的文件可复用。
- **模型参数集中到模型页**：服务商、模型名称、单次输出上限、四个 Agent 的温度、Top P、Top K、思考强度和上下文预算放在同一入口。每个 Agent 单独折叠，Writer 默认展开，避免高级页和语音页重复出现相同设置。
- **实际效率按 Agent 展示**：模型页按 Coordinator、Writer、Reviewer、Memory Keeper 显示真实调用次数、输入/输出 Token、平均输出、缓存命中率、未命中率与上下文预算。服务商没有返回缓存字段时明确显示“未报告”，不会把未知数据当成 0%。
- **缓存前缀更稳定**：Context Packet 继续把长期规则放在前面、本章任务放在末尾；JSON Schema 改为稳定序列化。OpenAI 请求增加按 Agent、模型和输出结构区分的缓存键，DeepSeek 继续使用官方的相同前缀自动缓存。温度和 Top P 只影响生成取样，不再被误解为缓存命中的控制项。
- **章节长度容差可调**：默认允许目标字数上下 20% 浮动，可在创作设置中调为 10%～100%。同一个值同时用于 Writer 初稿范围、过短补写、过长压缩、修订提示和 Reviewer 前的确定性检查。
- **正文从人物立场出发**：Writer 在每个场景内部先锁定当前视角人物的目标、恐惧、已知信息和注意偏向，再选择环境与感官细节。人物不可能知道的动机不由旁白直接宣布，其他人的心理通过话音、动作和反应呈现。
- **描写更丰富但不过量**：声音、触感、温度、气味、光影与空间距离会轮换使用，并且必须影响人物判断、情绪或动作。形容词落到具体对象上，优先用准确动词、物件变化和身体反应，避免只增加空泛修饰。
- **中文标点节奏校正**：逗号用于确有关系的分句，“、”只用于真正并列的短项目，英文逗号不进入中文正文。Writer 会静默检查一口气串联多个动作和解释的长句；Reviewer 只在能引用代表性正文时把过密标点记为阅读体验建议，不按符号数量机械阻断章节。
- **设置界面减轻绕路**：高级页只保留本机开发能力；模型生成参数全部归入模型页，语音运行参数只在语音页维护。复杂统计采用折叠详情，常用状态和操作保持在第一层。

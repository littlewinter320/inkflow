...
1. 点击“模型设置”，填写兼容 DeepSeek/OpenAI 请求格式的地址、模型名和 API Key；Key 进入 Windows 凭据库、不写进小说文件，还可以用低成本探针检查密钥、地址和模型名是否真的连通。
2. 点击“新建小说”。完全没有书名、题材或主角时，直接选“我没有想法，AI 来构思”，Writer 会给出三个不同方向；你选中一个并确认后才建立本地项目。已有想法也可以切换为手动填写。

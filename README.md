...
> 当前状态：`0.3.1` 首个公开发布版。Windows 10/11 桌面端、VS Code 扩展与通用 MCP 是主要交付形态；仓库尚未发布到 PyPI。
...
从 GitHub 的 Releases 页面下载 `InkFlow-Setup-0.3.1.exe`，双击完成安装，再打开“墨流 InkFlow”。第一次使用时：

> 当前公开安装包尚未配置商业代码签名证书，Windows 可能显示“未知发布者”或 SmartScreen 提示。可先用 Release 附件中的 `SHA256SUMS-0.3.1.txt` 核对文件；后续正式商业分发应接入可信代码签名证书。
...
安装包自带本地引擎，不要求普通用户安装 Python、Node 或使用终端。`inkflow-vscode-0.3.1.vsix` 是可选的 VS Code 适配器：在 VS Code 的“扩展”页面选择“从 VSIX 安装”，即可获得中文创作主控、小说树、选区批注/修订、审查、检查点和 MCP 配置入口。

桌面版包含“检查更新”界面。当前构建已指向本仓库的公开 GitHub Release；它可以在软件内检查、下载并在重启时安装新版本，不会删除用户选择的小说项目。发布维护说明见 [更新发布指南](docs/UPDATE_RELEASE_GUIDE.md)。
...
需要追问时，桌面版会弹出独立提问卡：每题说明“为什么问”，支持单选或多选，并在最后固定提供“其他”自由填写。VS Code 使用同样的结构化问题，通过编辑器原生选择框收集答案。用户也可以主动点击“先问我”；回答会以自然语言继续原会话，不要求记住任何触发词。

## 写作技能与联网资料

0.3 把重复写作方法整理成引导型技能，而不是禁词表：自然中文正文技能说明如何从场景目标写到可读正文；连载结构技能说明如何制作四级规划、轮换钩子并维护伏笔的种下、推进、转义、兑现和延期；资料研究技能说明何时搜索、如何判断来源，以及怎样把事实转成可执行写法。技能适配 Claude Code 与 TRAE，仍由原有三个小说角色执行，不增加第四个 Agent 或额外模型调用。

右侧“参考资料”页新增公开搜索。搜索阶段只把用户填写的搜索词发送给公开搜索页，先返回标题、摘要和来源，不自动保存；只有用户点击“导入这页”后，墨流才抓取并保存到当前项目。它适合查写作教学、陌生职业、历史事实和心理学资料，不会每章自动联网，也不会上传小说正文、正史或模型密钥。对应 MCP 工具是 `novel_reference_search`。

Context Builder 会把最近章节全文、与当前任务相关的旧章摘要、正史索引、参考特征卡和最多两条本章技法引导编译为一个上下文包。旧章摘要使用本地轻量相关性排序，不调用模型、不新增向量数据库；因此能减少“全部历史反复塞入”的 Token 浪费，同时保留可追踪的来源编号。
...
升级到 0.3 **不迁移正史数据库 `.inkflow/inkflow.db`，也不改变小说项目的用户可见目录结构**。桌面版继续使用独立的 `.inkflow/studio.db` 保存文档版本、批注、手工故事圣经、场景笔记和任务记录；0.3 新增的提问卡、写作技能与公开搜索不需要迁移。更新源码后，在工作区执行一次：
...
构建结果位于 `desktop/release/` 与 `vscode-extension/release/`。完整的 0.3 能力、升级影响和已知边界见 [0.3 发布说明](./docs/V0.3_RELEASE_NOTES.md)。

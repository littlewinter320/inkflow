...
| 能力 | 默认方案 | 使用方式 |
| --- | --- | --- |
| 普通话输入 | SenseVoice | 用户主动下载语音组件后使用 |
| 日常朗读 | Kokoro 中文版 | 默认朗读路径，支持多种男女声、速度、音量和停顿调整 |
...
Kokoro 和 Qwen 不会随普通安装强制下载。下载模型、占用显卡和创建声音档案都必须由用户主动操作；用户录音、克隆档案、角色映射和听读结果保存在应用语音目录中，不写入小说正史、Trace 或公共训练数据。
...
0.6.0 的重点是提高长篇创作的可控性、可理解性和本地语音体验：

- 增加章节结尾策略、Writer 钩子说明、Reviewer 钩子审查和结尾重复风险提示；
- 支持统一上下文预算与按 Agent 自定义预算，新增 Writer 上下文清单和 Reviewer 来源使用审计；
- 将 Reviewer 的硬门禁与阅读体验建议分开，支持精简、标准和详细建议；
- 新增逐章确认、批次确认一次、审查通过后自动验收三种策略；
- 批量写作使用可失效、可重建的临时连续性记忆，不会提前污染正史；
- 桌面端补充任务过程、版本状态、来源、错误影响和下一步操作；
- 默认日常朗读改为 Kokoro，普通话输入统一为 SenseVoice，可选 Qwen3-TTS 1.7B 高品质朗读与授权声音克隆；
- Windows 更新包支持差分下载条件，减少后续小版本更新的下载量。

更具体的安装包更新说明会随每个 GitHub Release 发布在下载页。
...
```powershell
# 安装 Python 引擎依赖
.\.venv\Scripts\python.exe -m pip install -e ".\agent[build,lightvoice]"

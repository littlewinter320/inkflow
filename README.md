...
当前版本：**0.6.2**。Windows 安装包、差分更新文件和完整更新说明都放在 [GitHub Releases](https://github.com/littlewinter320/inkflow/releases/latest)；源码版本、Release 标签和安装包版本保持一致。
...
0.6.2 是 0.6.1 的语音资源管理补丁：

- **Qwen 可卸载**：设置中的 Qwen 组件在安装后显示独立的“删除 Qwen”按钮；删除前仍会说明影响并要求确认。
- **删除范围明确**：删除只涉及 Qwen 隔离依赖、预设/克隆模型缓存和安装状态；录音、声音档案、任务记录及已生成音频继续保留。
- **引擎自动回退**：如果删除时当前正在使用 Qwen，系统会把朗读引擎切回默认的 MOSS，避免下次朗读进入不可用状态。
- **安装包保持轻量**：Qwen 和 MOSS 仍然只在用户主动点击安装时下载，普通安装包不携带这些大模型依赖。

## 0.6.1 核心更新

0.6.1 主要修复桌面端的交互断点，并让状态信息更接近真实运行情况：

- **语音输入即时反馈**：优先使用浏览器原生中文连续听写，录音过程中显示实时预览；停止后只把最终文字放回输入框，用户检查后自行发送，绝不自动提交。
- **运行中可继续对话**：生成进行时输入框和快捷提示仍可用；点击发送会一次性发出运行中引导并立即显示在对话里，取消了重复确认。
- **判断摘要改为公开证据卡**：结果默认只显示结论；展开后可查看公开判断摘要、工具返回和证据列表，不暴露模型私有思维链。
- **上下文容量状态修复**：Context Packet 记录项目资料修订标识；章节、计划、正史或参考资料变化后，面板会标记“资料已变化，等待重新编译”，不再继续显示过期数字。
- **项目工作台可读性提升**：项目页增加轻量 SVG 指标卡，设置弹窗、焦点状态和信息层级统一，保持零新增图表依赖。
